...
public interface ChangeCalculator {
    ...
    static void bankNoteOrCoin(double change, BankNoteOrCoin[] bankNoteOrCoins) {

        for (BankNoteOrCoin bankNoteOrCoin : bankNoteOrCoins) {
            int noteOrCoinTotal = (int) ((int) change / bankNoteOrCoin.value);
            if (noteOrCoinTotal > 0) {
                change = change % (int) (bankNoteOrCoin.value * 100);
